// 🔒 Solution 1: Force HTTPS for mobile networks
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 🧹 Solution 2: Disable any old Service Workers
    ...
    // ✅ Solution 7: Real connection check
    window.addEventListener('online', () => console.log('✅ Connection available'));
    window.addEventListener('offline', () => alert('⚠️ No internet connection'));
    
    // ✅ Solution 8: Direct test from phone
    ...
    // ✅ Solution 3: Configure Firestore for weak networks
    try {
        db.settings({
        ...
        console.log("Firestore settings already applied or failed:", e);
    }
    ...
    auth.onAuthStateChanged(async user => {
        if (user) {
            ...
            if (authContainer) {
                ...
                let avatarContent = avatar.startsWith('http') ? `<img src="${avatar}" alt="Profile Picture" loading="lazy">` : avatar;
    ...
    function createAdCard(ad, ownerProfile) {
        ...
        const avatarDisplay = ownerAvatar.startsWith('http') ? `<img src="${ownerAvatar}" alt="${ownerName}" loading="lazy">` : ownerAvatar;
    ...
    async function populateAdDetailsPage() {
        ...
        try {
            ...
            let avatarDisplay = avatar.startsWith('http') ? `<img src="${avatar}" alt="${creatorName}" loading="lazy">` : avatar;
        ...
        try {
            ...
            for (const doc of sortedDocs) {
                ...
                const avatarDisplay = reviewerAvatar.startsWith('http') 
                    ? `<img src="${reviewerAvatar}" alt="${reviewerName}" class="avatar-img" loading="lazy">`
    ...
    async function populateAccountPage(userId, userProfile, isOwnProfile) {
        ...
        const largeAvatarDiv = document.getElementById('profileAvatarLarge');
         if (largeAvatarDiv) {
            ...
            largeAvatarDiv.innerHTML = avatar.startsWith('http') ? `<img src="${avatar}" alt="Profile Picture" loading="lazy">` : avatar;
        }
